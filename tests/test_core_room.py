...
class TestRoom:
    ...
    def resources(self):
        ...
        room_id = create_room("test_room", user.id)
    ...
    def test_create_room_user_not_found(self):
        # Assert not exists user with id 1s
        assert not User.exists(id=NOT_EXISTS_ID)
    ...
    def test_get_rooms(self, resources):
        ...
        room_id = create_room("test_temporal_room", temp_user.id)
        # Get the rooms
        rooms = get_rooms()
        # Check if the rooms have the room created
        assert len(rooms) == 2
        # Delete the temporal room
        delete_room(room_id, temp_user.id)
        # Delete the user
    ...
    def test_delete_room(self):
        ...
        room_id = create_room("test_temporal_room", temp_user.id)
        # Check if the room has been created
        assert Room.exists(id=room_id)
        # Check if the room have the user as host
        assert room_id == temp_user.room.id
        # Delete the room
        delete_room(room_id, temp_user.id)
        # Check if the room has been deleted
        assert Room.get(id=room_id) is None
        # Delete the user
        delete_user(temp_user.id)
    ...
    def test_join_room(self):
        ...
        # Join the room
        join_room(room_id, temp_user2.id)
        # Check if the user has joined the room
    ...
    def test_leave_room_user_host(self):
        ...
        temp_user = create_user("temp_user")
        temp_room_id = create_room("temp_room", temp_user.id)
        # Leave the room with the host
        leave_room(temp_room_id, temp_user.id)
        # Check if the room has been deleted
        assert Room.get(id=temp_room_id) is None
        # Delete the user
        delete_user(temp_user.id)
    ...
    def test_leave_room_user_host_with_users(self):
        ...
        temp_user2 = create_user("temp_user2")
        # Create a room
        temp_room_id = create_room("test_room", temp_user.id)
        # Join the room
        join_room(temp_room_id, temp_user2.id)
        # Leave the room with the host
        leave_room(temp_room_id, temp_user.id)
        # Check if the room has been deleted
        assert Room.get(id=temp_room_id) is None
    ...
    def test_start_game_user_not_host(self):
        ...
        with pytest.raises(PermissionError):
            start_game(temp_room_id, temp_user2.id)
        # Delete the room
        delete_room(temp_room_id, temp_user.id)
        # Delete the users
        delete_user(temp_user.id)
        delete_user(temp_user2.id)
    ...
    def test_start_game_room_full(self):
        ...
        join_room(temp_room_id, temp_user2.id)
        # Join the room
        join_room(temp_room_id, temp_user3.id)
        # Start the game
        with pytest.raises(PermissionError):
            start_game(temp_room_id, temp_user.id)
        # Delete the room
        delete_room(temp_room_id, temp_user.id)
        # Delete the users
        delete_user(temp_user.id)
