...
# ================================== CHANGE OF POSITION / YOU BETTER RUN ==================================
...
class TestYouBetterRun:
    ...
    def setup_method(self):
        ...
        draw_specific(id_game=1, id_player=1, idtype_card=12)
    ...
    def teardown_method(self):
        """Teardown method."""
        Game[1].current_phase = "Discard"
        discard(id_game=1, id_player=1, idtype_card=12)
    ...
    def test_you_better_run_effect(self):
        ...
        assert modified_game_data["current_position"] == 3

        # With modifications in the players (consider that the positions are 0-indexed)
        assert modified_game_data["players"][0]["round_position"] == 3
        assert modified_game_data["players"][2]["round_position"] == 1

        # Check that the changes are only it
        modified_game_data["current_position"] = 1
        modified_game_data["players"][0]["round_position"] = 1
        modified_game_data["players"][2]["round_position"] = 3

        assert game_data == modified_game_data

    @db_session
    def test_you_better_run_effect_with_dead_player(self):
        """Test you better run effect with dead player."""
        Player[11].alive = False
        commit()
