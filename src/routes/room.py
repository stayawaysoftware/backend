...
async def create_room(name: str, host_id: int, min_users: int = 4, max_users: int = 12):
    with db_session:
        ...
        if User.get(id=host_id).lobby is not None:
            raise HTTPException(status_code=500, detail="User is already in a room")
        if min_users < 4 or min_users > 12 or min_users > max_users:
            raise HTTPException(status_code=500, detail="Minimum users invalid value")
        if max_users < 4 or max_users > 12 or max_users < min_users:
            raise HTTPException(status_code=500, detail="Maximum users invalid value")
        
        room = Room(name=name, host_id=host_id, min_users=min_users, max_users=max_users)
